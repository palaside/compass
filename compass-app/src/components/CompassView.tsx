    ...
    try {
      ...
      if (typeof DeviceOrientationEvent !== 'undefined' && typeof (DeviceOrientationEvent as any).requestPermission === 'function') {
        ...
        if (response === 'granted') {
          ...
          alert('อนุญาตสิทธิ์สำเร็จ (Granted) - เข็มทิศจะหมุนตามอุปกรณ์');
        } else {
          setPermissionGranted(false);
          alert(`ไม่อนุญาตสิทธิ์ สถานะ: ${response}`);
        }
      } else {
        // Fallback for Android / browsers that don't need requestPermission
        setPermissionGranted(true);
        setUseManualSlider(false);
        alert('เปิดใช้งานเซนเซอร์สำเร็จ (อุปกรณ์นี้ไม่ต้องขอสิทธิ์เพิ่มเติม)');
      }
    } catch (err) {
  ...
  return (
    <div className="relative w-full h-full bg-[#020703] text-[#CEDE62] flex flex-col justify-between overflow-hidden font-mono select-none">
      ...
      <div className="relative z-20 w-full bg-[#121413]/95 backdrop-blur-md border-b border-[#233527] px-3 py-2.5 sm:px-6 shadow-2xl">
        ...
        {/* Mobile Sub-toolbar for Tactical Lines, True North, Manual Sim & Sensor Permission */}
        <div className="flex sm:hidden flex-wrap items-center justify-center mt-2 pt-1.5 border-t border-gray-800 text-xs gap-1.5">
          ...
          >
            ...
            <span>{permissionGranted ? 'เซนเซอร์: ON' : 'ขอสิทธิ์เซนเซอร์'}</span>
          </button>
